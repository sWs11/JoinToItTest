...
@section('content')
    <div class="container mt-3">
        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header">{{ __('main.add_employee') }}</div>
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('employees.update', $employee->id) }}">
                            ...
                            <div class="form-group row">
                                <label for="company_id" class="col-md-4 col-form-label text-md-right">{{ __('main.company_name') }}</label>

                                <div class="col-md-6">
                                    <select class="custom-select" id="company_id" name="company_id">
                                        <option value="{{ $employee->company_id }}">{{ $employee->company_name }}</option>
                                    </select>
                                </div>
...
@push('scripts')
    ...
    <script !src="">
        const select_url = "{{ route('companies.getCompanies') }}";
    </script>
    <script src="{{ asset('js/employee_select_script.js') }}"></script>
@endpush
